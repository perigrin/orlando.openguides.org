...
sub recent_changes {
  ...
  foreach my $change (@changes)
  {
    ...
    my $time = Time::Piece->strptime($change->{timestamp}, $self->{timestamp_fmt});

    my $utc_offset = $self->{utc_offset};
    
    $change->{timestamp} = $time->strftime( "%Y-%m-%dT%H:%M:%S$utc_offset" );

    $change->{author}      = $change->{metadata}{username}[0] || $change->{metadata}{host}[0] || '';
    ...
    $change->{url} = $self->{make_node_url}->($change->{name}, $change->{version});
    ...
    if ($self->{make_history_url}) {
      $change->{history_url} = $self->{make_history_url}->($change->{name});
    }
...
=head1 NAME
  ...
  Wiki::Toolkit::Plugin::JSON - A Wiki::Toolkit plugin to output RecentChanges RSS.
...
=head1 SYNOPSIS
  ...
  my $rss = Wiki::Toolkit::Plugin::RSS::ModWiki->new(
    wiki                => $wiki,
    site_name           => 'My Wiki',
